...
from .contract_specs import FUTURES_CONTRACT_SPECS, get_point_value_from_contract
...
def get_base_symbol(contract_name: str) -> Optional[str]:
    """Symbole racine dans FUTURES_CONTRACT_SPECS (ex. NQM6 -> NQ, MNQ -> MNQ)."""
...
def resolve_point_value_for_contract(contract_name: str, trade_point_value: Any = None) -> Optional[Decimal]:
    """Valeur du point : specs sur symbole normalisé, puis champ trade en secours."""
    from_specs = get_point_value_from_contract(contract_name)
    if from_specs is not None:
        return Decimal(str(from_specs))
