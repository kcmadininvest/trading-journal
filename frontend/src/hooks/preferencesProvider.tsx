...
export const PreferencesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const applyPrefsFromServer = useCallback((prefs: UserPreferences) => {
    if (!prefs?.date_format) return;
    setPreferences({
      ...prefs,
  ...
  useEffect(() => {
    if (bootstrap?.preferences) {
      applyPrefsFromServer(bootstrap.preferences);
      setLoading(false);
    }
  }, [bootstrap, applyPrefsFromServer]);
  ...
  useEffect(() => {
    ...
    if (bootstrapError || (!bootstrapLoading && !bootstrap?.preferences)) {
      void refreshPreferences();
    }
  }, [bootstrapError, bootstrapLoading, bootstrap?.preferences, refreshPreferences]);
  ...
  }, [applyPrefsFromServer, refreshPreferences]);
