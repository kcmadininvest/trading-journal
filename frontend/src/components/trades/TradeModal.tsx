    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-xl rounded-lg shadow">
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Modifier le trade</h2>
        ...
        <div className="px-4 py-3 border-t flex items-center justify-end gap-2">
          <button onClick={() => onClose(false)} className="px-3 py-2 rounded bg-gray-100">Annuler</button>
          <button onClick={save} disabled={isSaving} className="px-3 py-2 rounded bg-blue-600 text-white disabled:opacity-50">Enregistrer</button>
        </div>
      </div>
    </div>
  );
