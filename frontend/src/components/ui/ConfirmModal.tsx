...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  const defaultTitle = title || t('confirmModal.title', { defaultValue: 'Confirmation' });
  const defaultMessage = message || (
    <p className="text-gray-600 dark:text-gray-400">
      {t('confirmModal.message', { defaultValue: 'Voulez-vous continuer ?' })}
    </p>
  );
  const defaultConfirmText = confirmButtonText || t('confirmModal.confirm', { defaultValue: 'Confirmer' });
  const defaultCancelText = cancelButtonText || t('common:cancel', { defaultValue: 'Annuler' });
        ...
        <div className="p-6">
          <div className="mb-6">
            {typeof defaultMessage === 'string' ? (
              <p className="text-gray-600 dark:text-gray-400 whitespace-pre-line">{defaultMessage}</p>
            ) : (
              defaultMessage
            )}
          </div>
